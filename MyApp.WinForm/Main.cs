...
namespace MyApp.WinForm
{
    public partial class Main : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            lstUsers.Columns.Add("Date of Birth", 100);
        }
        ...
        private void btnActiveUsers_Click(object sender, EventArgs e)
        {
            ...
            // Filter users whose IsActive field is set to true
            var nonActiveUsers = users.Where(user => user.IsActive).ToList();

            // pass the filtered list to ListView
            LoadListView(nonActiveUsers); 
        }
       ...
       // Get the users and filter by NonActive
        private void btnNonActiveUsers_Click(object sender, EventArgs e)
        {
            ...
            // Filter users whose IsActive field is set to false
        ...
        {
            var addUserForm = new AddUser(ServiceFactory);
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            if (lstUsers.SelectedItems.Count > 0)
            {
                ...
                // Create new form
                var editUserForm = new EditUserForm(ServiceFactory, userId);
                ...
                {
                    // Refresh the user list after the user is edited.
                    var users = ServiceFactory.UserService.GetAll();
                    LoadListView(users);
                }
            }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (lstUsers.SelectedItems.Count > 0)
            {
                ...
                User userToDelete = ServiceFactory.UserService.GetById(userId);

                // Create the DeleteUserForm and pass in the IServiceFactory and userToDelete
                var deleteUserForm = new DeleteUserForm(ServiceFactory, userToDelete);

                // Show the DeleteUserForm and check if the DialogResult is OK
                if (deleteUserForm.ShowDialog() == DialogResult.OK)
                {
                    // Refresh the user list after the user is deleted
                    var users = ServiceFactory.UserService.GetAll();
                    LoadListView(users);
                }
            }
